let config={activate:'g',bgColor:'#222b'};function configurate(d,f){config.hasOwnProperty(d)&&(config[d]=f)}try{let d=document.currentScript.dataset.json;if(d)try{let f=JSON.parse(document.currentScript.childNodes[0].data);for(let g in f)f.hasOwnProperty(g)&&configurate(g,f[g])}catch(f){console.log('JS Flashlight Configuration with JSON failed!'),console.log(f)}}catch(d){console.log('JS Flashlight configuration not used.')}class TransformationMatrix{constructor(){this.x=0,this.y=0,this.z=0,this.a=1,this.b=1,this.c=1}translate(d,f,g){this.x+=d,this.y+=f,this.z+=g}setTranslation(d,f,g){this.x=d,this.y=f,this.z=g}scale(d,f,g){this.a*=d,this.b*=f,this.c*=g}setScale(d,f,g){this.a=d,this.b=f,this.c=g}setCompleteScale(d){this.a=d,this.b=d,this.c=d}getMatrixAsArray(){return[this.a,0,0,this.x,0,this.b,0,this.y,0,0,this.c,this.z,0,0,0,1]}}class PerspectiveVector{constructor(d){this.x=1,this.y=d,this.z=1,this.w=1}updateAspectRatio(d){this.y=d}getVectorAsArray(){return[this.x,this.y,this.z,this.w]}}function fljs_fragment_shader_rect(){return`#version 300 es
    ...
    `}function setUniformTransformationMatrix(d,f,g){d.useProgram(f);let h=d.getUniformLocation(f,'transformationMatrix');d.uniformMatrix4fv(h,!1,new Float32Array(g))}function setUniformBGColor(d,f,g){d.useProgram(f);let h=d.getUniformLocation(f,'bg_color');d.uniform4fv(h,new Float32Array(g))}function setUniformPerspectiveVector(d,f,g){d.useProgram(f);let h=d.getUniformLocation(f,'perspectiveVector');d.uniform4fv(h,new Float32Array(g))}let transformationMatrix=new TransformationMatrix,perspectiveVector,square,simpelShader,realShader,simpelRectShader,realRectShader;function loadShader(d,f,g){let h='fragment'===g?d.createShader(d.FRAGMENT_SHADER):'vertex'===g?d.createShader(d.VERTEX_SHADER):null;return null==h?null:(d.shaderSource(h,window[f]()),d.compileShader(h),d.getShaderParameter(h,d.COMPILE_STATUS)?h:(console.log('Shader Compilation failed: '+f+d.getShaderInfoLog(h)),null))}function initShaderProgram(d,f){let g=loadShader(d,f,'fragment'),h=loadShader(d,'fljs_vertex_shader','vertex'),i=d.createProgram();d.attachShader(i,h),d.attachShader(i,g),d.linkProgram(i),d.getProgramParameter(i,d.LINK_STATUS)||console.log('Shader initialisation failed: '+f),d.useProgram(i);let j=d.getAttribLocation(i,'aVertexPosition');return d.enableVertexAttribArray(j),i}function initSquareBuffer(d){let f=d.createBuffer();d.bindBuffer(d.ARRAY_BUFFER,f);return d.bufferData(d.ARRAY_BUFFER,new Float32Array([1,1,0,-1,1,0,1,-1,0,-1,-1,0]),d.STATIC_DRAW),f}function getCurrentShader(){return 1===fl_type?simpelShader:2===fl_type?realShader:4===fl_type?realShader:5===fl_type?simpelRectShader:6===fl_type?realRectShader:7===fl_type?simpelRectShader:8===fl_type?realRectShader:simpelShader}let drawWebGL2=function(d){let f=getCurrentShader(),g=-(window.innerWidth/2-fl_data.x)/(window.innerWidth/2),h=(window.innerHeight/2-fl_data.y)/(window.innerHeight/2);transformationMatrix.setTranslation(g,h,0);let i=fl_data.size/fl_data.default;transformationMatrix.setScale(i,i*fl_data.height,i),d.useProgram(f),d.clear(d.COLOR_BUFFER_BIT|d.DEPTH_BUFFER_BIT),setUniformTransformationMatrix(d,f,transformationMatrix.getMatrixAsArray()),setUniformPerspectiveVector(d,getCurrentShader(),perspectiveVector.getVectorAsArray()),d.bindBuffer(d.ARRAY_BUFFER,square),d.vertexAttribPointer(d.getAttribLocation(f,'aVertexPosition'),3,d.FLOAT,!1,0,0),d.drawArrays(d.TRIANGLE_STRIP,0,4)};function hexToArray(d){if(d.startsWith('#')){let f=parseInt(d.substr(1),16),g=4;7<=d.length&&(g=8);let h=g*g-1;return[(f>>3*g&h)/(h+1),(f>>2*g&h)/(h+1),(f>>g&h)/(h+1),(f&h)/(h+1)]}return console.log('JS Flashlight: Invalid HEX String! '+d),[0,0,0,0]}function initWebGL(d){let f=d.getContext('webgl2');if(!f)return console.log('JS Flashlight: No WebGL2 context found'),!1;console.log('JS Flashlight using WebGL2'),draw=drawWebGL2,fl_data.size=40,fl_type=3;let g=hexToArray(config.bgColor);return f.clearColor(g[0],g[1],g[2],g[3]),f.clear(f.COLOR_BUFFER_BIT),square=initSquareBuffer(f),simpelShader=initShaderProgram(f,'fljs_fragment_shader_real_simple'),realShader=initShaderProgram(f,'fljs_fragment_shader_real'),simpelRectShader=initShaderProgram(f,'fljs_fragment_shader_rect'),realRectShader=initShaderProgram(f,'fljs_fragment_shader_rect_real'),transformationMatrix.setCompleteScale(fl_data.size/200),transformationMatrix.translate(0,0,-1),setUniformBGColor(f,simpelShader,g),setUniformBGColor(f,realShader,g),setUniformBGColor(f,simpelRectShader,g),setUniformBGColor(f,realRectShader,g),perspectiveVector=new PerspectiveVector(d.clientWidth/d.clientHeight),setUniformPerspectiveVector(f,getCurrentShader(),perspectiveVector.getVectorAsArray()),window.addEventListener('resize',function(){ctx.viewport(0,0,f.drawingBufferWidth,f.drawingBufferHeight),perspectiveVector.updateAspectRatio(f.drawingBufferWidth/f.drawingBufferHeight)},!1),f}document.addEventListener('mousemove',onMouseMove,!1),document.addEventListener('keyup',onKeyUp,!1),document.addEventListener('keydown',onKeyDown,!1),document.addEventListener('mousewheel',onMousewheel,!1),document.addEventListener('DOMMouseScroll',onMousewheel,!1),window.addEventListener('resize',onWindowResize,!1);let pressed_keys=[],fl_active=!1,fl_type=2,fl_data={x:-1,y:-1,size:150,'default':150,height:1},draw,drawCanvas2D=function(d){0<=fl_type&&drawBackground(d),1<=fl_type&&drawSimpleFl(d),2<=fl_type&&drawRealShadow(d)},canvas_fl=createCanvas(),ctx=!1;'function'==typeof initWebGL&&(ctx=initWebGL(canvas_fl)),ctx||(ctx=initCanvas2D());function initCanvas2D(){console.log('JS Flashlight using Canvas 2D');let d=canvas_fl.getContext('2d');return draw=drawCanvas2D,d}function onMousewheel(d){if(fl_active){let f=Math.max(-1,Math.min(1,d.wheelDelta||-d.detail));if(d.shiftKey&&!d.altKey)fl_data.size+=4*f;else if(d.altKey&&!d.shiftKey)fl_data.height+=0.05*f;else if(d.shiftKey&&d.altKey)fl_data.size-=1.1*f,fl_data.height+=0.2*f;else return void draw(ctx);d.preventDefault(),draw(ctx)}}function onKeyUp(d){if(pressed_keys[d.keyCode]=0,d.key===config.activate){let f=window.getComputedStyle(canvas_fl),g=f.getPropertyValue('opacity');0==g?(canvas_fl.style.opacity='1',fl_active=!0):(canvas_fl.style.opacity='0',setTimeout(function(){fl_active=!1},500))}}function onKeyDown(d){pressed_keys[d.keyCode]=1,swapFlModes(d.key)}function isKeyDown(d){return 1===pressed_keys[d.charCodeAt(0)]}function swapFlModes(d){'0'===d?(fl_type=0,fl_data.height=1):'1'===d?(fl_type=1,fl_data.height=1):'2'===d?(fl_type=2,fl_data.height=1):'3'===d?(fl_type=3,fl_data.height=window.innerHeight/window.innerWidth):'4'===d?(fl_type=4,fl_data.height=window.innerHeight/window.innerWidth):'5'===d?(fl_type=5,fl_data.height=window.innerHeight/window.innerWidth):'6'===d?(fl_type=6,fl_data.height=window.innerHeight/window.innerWidth):'7'===d?(fl_type=7,fl_data.height=1):'8'===d?(fl_type=8,fl_data.height=1):void 0;draw(ctx)}function onMouseMove(d){fl_active&&(fl_data.x=d.clientX,fl_data.y=d.clientY,draw(ctx))}function drawRealShadow(d){d.beginPath(),d.globalCompositeOperation='source-over';let f=d.createRadialGradient(fl_data.x,fl_data.y,0,fl_data.x,fl_data.y,fl_data.size);f.addColorStop(0,'#0000'),f.addColorStop(0.83,'#0000'),f.addColorStop(0.88,'#00f1'),f.addColorStop(0.93,'#1c3f8644'),f.addColorStop(0.96,'#795e8688'),f.addColorStop(1,'#795e8600'),d.fillStyle=f,d.arc(fl_data.x,fl_data.y,fl_data.size,0,2*Math.PI,!1),d.fill()}function drawSimpleFl(d){d.beginPath(),d.globalCompositeOperation='destination-out';let f=d.createRadialGradient(fl_data.x,fl_data.y,0,fl_data.x,fl_data.y,fl_data.size);f.addColorStop(0,'white'),f.addColorStop(0.78,'white'),f.addColorStop(1,'transparent'),d.fillStyle=f,d.arc(fl_data.x,fl_data.y,fl_data.size,0,2*Math.PI,!1),d.fill()}function drawBackground(d){d.globalCompositeOperation='source-over',d.clearRect(0,0,canvas_fl.width,canvas_fl.height),d.fillStyle=config.bgColor,d.fillRect(0,0,canvas_fl.width,canvas_fl.height)}function onWindowResize(){canvas_fl.width=window.innerWidth,canvas_fl.height=window.innerHeight,draw(ctx)}function createCanvas(){let d=document.createElement('canvas');return d.style.position='fixed',d.style.left='0',d.style.top='0',d.style.opacity='0',d.style.transition='all 0.5s ease-in-out',d.style.zIndex='2147483647',d.style.pointerEvents='none',d.style.background='transparent',d.width=window.innerWidth,d.height=window.innerHeight,document.body.appendChild(d),d}